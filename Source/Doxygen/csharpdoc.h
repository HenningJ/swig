...
/*
 * A class to translate doxygen comments into CSharp XML documentation style comments.
 */
...
protected:
  /*
   * Used to properly format CSharpDoc-style command
   */
  ...
  /**
  * Creates a <summary> tag
  */
  void handleSummary(DoxygenEntity &tag, std::string &translatedComment, std::string &);
   ...
   * 
   */
  void handleTagCref(DoxygenEntity &tag, std::string &translatedComment, std::string &arg);
  ...
  /*
   * Print extended CSharpdoc command, like {@code ...} or {@literal ...}
   * arg - command name
   */
